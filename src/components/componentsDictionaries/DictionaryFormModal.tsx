// Универсальная модалка создания/редактирования узла иерархического справочника
...
interface DictionaryFormModalProps {
    ...
    onClose: () => void;
    mode: "create" | "edit";
    initialValues: HierarchicalDictFormValues;
    parentOptions: ParentTreeOption[];
    submitting: boolean;
    error: string | null;
    onSubmit: (values: HierarchicalDictFormValues) => void;
    // Префикс ключей i18n конкретной страницы
    pageKey: string;
}

export function DictionaryFormModal({
                                        open,
                                        onClose,
                                        mode,
                                        initialValues,
    ...
    const [values, setValues] = useState<HierarchicalDictFormValues>(initialValues);
            ...
            >
                ...
                <div className="p-5 flex flex-col gap-3.5">
                    ...
                    <ParentMultiSelectField
                        label={t(`${pageKey}.fieldParent`)}
                        modalTitle={t(`${pageKey}.fieldParent`)}
                        options={parentOptions}
                        selectedKeys={values.parentKeys}
                        onChange={(keys) => setValues((v) => ({...v, parentKeys: keys}))}
                        searchPlaceholder={t("general.search")}
                    />

                    {values.parentKeys.length > 1 && (
                        <p className="m-0 text-[11.5px] text-[#8b97ab] leading-[1.5]">
                            {t("dictionaries.multiParentHint")}
                        </p>
                    )}
