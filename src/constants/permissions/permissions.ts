// Зеркало backend enum PermissionCode (Modules/Users/Models/PermissionCode.cs)
export const PermissionCode = {
    /* Просмотр страницы актуализации ВНД */
    ViewVndActualizationPage: 1,

    /* Взять любую ВНД в актуализацию с последующим согласованием (без запроса права) */
    ActualizeAnyVndWithApproval: 2,

    /* Взять любую ВНД в актуализацию без согласования (без запроса права) */
    ActualizeAnyVndWithoutApproval: 3,

    /* Взять ВНД в актуализацию с последующим согласованием (по запросу права) */
    ActualizeVndWithApprovalByRequest: 4,

    /* Взять ВНД в актуализацию без согласования (по запросу права) */
    ActualizeVndWithoutApprovalByRequest: 5,

    /* Создать новую ВНД с последующим согласованием */
    CreateVndWithApproval: 6,

    /* Создать новую ВНД без последующего согласования */
    CreateVndWithoutApproval: 7,

    /* Удалить ВНД */
    DeleteVnd: 8,

    /* Редактировать последнюю редакцию без согласования, без создания новой редакции и изменения даты актуализации */
    EditLastRevisionDirectly: 9,

    /* Управление группами */
    ManageGroups: 10,

    /* Просмотр ВНД */
    ViewVnd: 11,

    /* Экспорт ВНД */
    ExportVnd: 12,

    /* Управление пользователями */
    ManageUsers: 13,

    /* Управление ролями */
    ManageRoles: 14,

    /* Просмотр ограниченной статистики */
    ViewLimitedStatistics: 15,

    /* Экспорт отчёта по полной статистике */
    ExportFullStatisticsReport: 16,

    /* Просмотр полной статистики */
    ViewFullStatistics: 17,

    /* Возможность выступать в роли согласующего */
    ActAsApprover: 18,

    /* Возможность изменять маршрут согласования (удалять лишних пользователей) */
    ModifyApprovalRoute: 19,

    /* Возможность просматривать черновики других пользователей */
    ViewOtherUsersDrafts: 20,

    /* Управление справочниками ВНД (Виды ВНД, Уровни секретности, Группы пользователей, Рубрикатор, Обязательные участники согласования) */
    ManageVndDictionaries: 21,

    /* Управление общими справочниками (Органы утверждения, Структурные подразделения, Должности) */
    ManageGeneralDictionaries: 22,

    /* Управление справочниками служебных записок (Категории СЗ) */
    ManageSzDictionaries: 23,

    /* Управление справочниками закупок (Чёрный список контрагентов, Пороги закупок) */
    ManageProcurementDictionaries: 24,

    /* Изменение реквизитов существующей ВНД и её связей с другими документами */
    EditVndRequisites: 25,

    /* Доступ к разделу «Заседания». Полную повестку видят только члены органа и его секретарь */
    ViewMeetings: 26,

    /* Ведение заседаний Правления (Секретарь Правления) */
    ManageBoardMeetings: 27,

    /* Ведение заседаний КПА (Секретарь КПА) */
    ManageKpaMeetings: 28,

    /* Ведение заседаний Кредитного комитета */
    ManageCreditCommitteeMeetings: 29,

    /* Заполнение отчёта об исполнении и статуса поручений по протоколам */
    ReportMeetingExecution: 30,

    /* Выгрузка реестра решений комитетов в Excel */
    ExportMeetingRegistry: 31,

    /* Член Правления — полная повестка заседаний Правления */
    MemberOfBoard: 32,

    /* Член КПА — полная повестка заседаний КПА */
    MemberOfKpa: 33,

    /* Член Кредитного комитета — полная повестка заседаний КИТ */
    MemberOfCreditCommittee: 34,
    /** Системные настройки: интеграции, расписание синхронизации */
    ManageSystemSettings: 35,

    /** Просмотр реестра ВНД в расширенном режиме: колонки и фильтры
     * "Статус последней редакции" и "Актуализация" */
    ViewVndRegistryExtended: 36,

    /** Просмотр реестра доверенностей — «вправе ли он это подписать» */
    ViewPowersOfAttorney: 37,

    /** Ведение доверенностей: выдача, отзыв, передоверие */
    ManagePowersOfAttorney: 38,

    /** Просмотр книги регистрации корреспонденции */
    ViewCorrespondence: 39,

    /** Регистрация писем и ведение справочника корреспондентов */
    RegisterCorrespondence: 40,

    /** Запросы по счетам клиентов — банковская тайна, узкий круг */
    ViewBankSecrecyInquiries: 41,

    /** Просмотр книги приказов по личному составу */
    ViewHrOrders: 42,

    /** Издание кадровых приказов */
    ManageHrOrders: 43,

    /** Отозвать согласование любой ВНД, не будучи её инициатором */
    CancelAnyVndApproval: 44,

    /* Видеть все заявки на закупку, а не только свои */
    ViewAllProcurements: 45,

    /* Вести закупочную процедуру (Сектор закупок): конкурс, публикация, вскрытие */
    ConductProcurement: 46,

    /* Вносить решения комиссии по закупке (секретарь комиссии) */
    RecordCommissionDecisions: 47,

    /* Формировать протокол закупки */
    ManageProcurementProtocol: 48,

    /* Договоры по закупкам: акты, претензии, исполнение */
    ManageProcurementContracts: 49,

    /* Вести План закупок */
    ManageProcurementPlan: 50,

    /* Реестр поставщиков и чёрный список */
    ManageSuppliers: 51,

    /* Видеть все служебные записки банка */
    ViewAllSz: 52,

    /* Регистрировать служебные записки */
    RegisterSz: 53,

    /**
     * Консолидировать согласованную редакцию любой ВНД, не будучи ответственным за
     * актуализацию или инициатором согласования. Право главного методолога — см.
     * VndActualizationService.PublishAsync на бэке. Номер обязан совпадать с сервером.
     */
    ConsolidateAnyVnd: 54,

    /**
     * Архивировать (отменить) ВНД — кнопка "Архивировать", доступна на любом статусе, кроме
     * черновика (тот только удаляется) и уже архивированного. Если документ на согласовании —
     * оно отзывается автоматически в рамках той же операции, см. VndService.CancelAsync на
     * бэке. Номер обязан совпадать с сервером.
     */
    CancelVnd: 55,

    /** Вынести подписанную записку на коллегиальный орган — право председателя. */
    SubmitSzToBody: 56,

    /**
     * Настраивать рассылки плана актуализации ВНД — кнопка "Настройки рассылок плана" на
     * странице планирования актуализации (ActualizationPageHeader) и страница
     * /management/mailing-settings. Без этого права кнопка не рисуется. Номер обязан
     * совпадать с сервером.
     */
    ManageVndActualizationMailing: 57,

    /**
     * Видеть счётчик просроченных/критичных по сроку ВНД на бейдже пункта "Планирование
     * актуализации" в сайдбаре. Отдельно от ViewVndActualizationPage: то право открывает саму
     * страницу и со временем станет доступно всем ролям, а это управляет только кружком-
     * счётчиком. Номер обязан совпадать с сервером.
     */
    ViewVndActualizationOverdueBadge: 58,

    /**
     * Редактировать маршрут уже запущенного согласования ВНД: добавлять/убирать согласующих —
     * право главного редактора. Узкое право, отдельное от широкого набора "главного редактора"
     * (CreateVndWithApproval и т.п.) по той же причине, что и CancelAnyVndApproval. Номер
     * обязан совпадать с сервером.
     */
    EditAnyVndApprovalRoute: 59,

    /**
     * Видеть вкладку "Заявки на актуализацию" (ActualizationRequestsPanel) и решать по заявкам —
     * тот же круг лиц получает и уведомления о новых заявках. Узкое право, отдельное от
     * широкого набора "главного редактора" (ActualizeAnyVndWith(out)Approval) по той же причине,
     * что и EditAnyVndApprovalRoute — широкий набор есть и у ролей без отношения к рассмотрению
     * заявок (например, у администраторов). Номер обязан совпадать с сервером.
     */
    ApproveVndActualizationRequests: 60,

    /**
     * Получать и разбирать предложения по ВНД: уведомления о новых предложениях и страница
     * "Нормотворчество (ВНД)" → "Предложения по ВНД" (VndProposalsPage). Отправить предложение
     * (кнопка "+ Предложения по ВНД" на странице ВНД) может любой — отдельного права нет. Роль
     * главного редактора ВНД. Номер обязан совпадать с сервером.
     */
    ManageVndProposals: 61,
} as const;