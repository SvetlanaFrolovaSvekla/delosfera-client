// Палитра цветов для подсветки цитат согласующих в тексте редакции (см. useDocxQuoteMarks) -
// у каждого согласующего свой цвет, чтобы в тексте сразу было видно, КТО что процитировал, а не
// просто "что-то здесь процитировано" (как было раньше - единая жёлтая подсветка на всех).
//
// Ровно 10 цветов (так и просили) - при более чем 10 согласующих на одном маршруте цвета
// неизбежно начнут повторяться, это осознанное ограничение, а не попытка сделать палитру без
// границ.
...
/** Стабильно назначает согласующему (по его userId) один и тот же цвет из палитры - ОДИНАКОВЫЙ
 * во всех документах и при каждой перезагрузке страницы, а не "первый пришёл - первым получил
 * цвет" (это плавало бы каждый раз при смене порядка резолюций/данных). При больше чем 10
 * согласующих на маршруте цвета начинают повторяться - это ожидаемо (см. комментарий у палитры
 * выше), совпадающий цвет двух РАЗНЫХ согласующих в такой ситуации - приемлемый компромисс, а
 * не баг. */
 ...
 * из цветов ВСЕХ согласующих, чьи цитаты пересеклись в этом месте текста (см.
 * useDocxQuoteMarks - "как-то показать, что тут процитировали несколько человек в одном и том
 * же месте", вместо того чтобы произвольно выбирать чей-то один цвет и прятать остальных). */
