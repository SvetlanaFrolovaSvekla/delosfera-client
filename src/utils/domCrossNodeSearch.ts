 ...
 * (пробел/перенос строки/таб/неразрывный пробел) в запросе совпадает с ЛЮБОЙ последовательностью
 * пробельных символов в документе. Это защищает от расхождений в пробелах/переносах строк между
 * текстом, скопированным через выделение (Selection API схлопывает/по-разному расставляет
 * пробелы на границах абзацев/строк), и тем, как тот же текст на самом деле лежит в DOM.
 *
