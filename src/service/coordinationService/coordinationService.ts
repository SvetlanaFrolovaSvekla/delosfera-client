...
class CoordinationService {
    ...
    /** История всех процессов согласования ВНД — по всем редакциям и циклам актуализации,
     * включая завершённые/отозванные/отклонённые. Для таба "История" -> "Редакции и
     * юридическая значимость" (кто инициировал, кто и как согласовывал каждую редакцию). */
    async getHistory(vndId: number): Promise<ApprovalProcessResponse[]> {
        const { data } = await axiosInstance.get<ApprovalProcessResponse[]>(
