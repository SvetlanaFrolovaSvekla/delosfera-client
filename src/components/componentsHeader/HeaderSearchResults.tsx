// Результаты поиска на главном Header на странице
...
export function HeaderSearchResults({results, loading, query, onSelect}: HeaderSearchResultsProps) {
    ...
    return (
        <div
            className="absolute left-0 right-0 top-[calc(100%+6px)] bg-white border border-[#e5e9f0] rounded-xl shadow-lg z-50 overflow-hidden">
            {loading ? (
                <div className="px-4 py-3 text-[12.5px] text-[#8b97ab]">
                    {/* Поиск… */}
                    {t("header.searching")}
                </div>
            ) : results.length === 0 ? (
                        ...
                        return (
                            <li key={r.id}>
                                ...
                                >
                                    ...
                                    <span
                                        className="flex-1 min-w-0 block text-[12.5px] font-medium text-[#1c2740] whitespace-normal break-words">
                                        <span className="font-mono font-semibold text-[#4e57d6]">
                                            {/* ВНД <HighlightText text={r.code} query={query}/> */}
                                            {t("vnd.shortLabel")} <HighlightText text={r.code} query={query}/>
                                        </span>
                                        {" — "}
