...
interface Props {
    /** Сообщить списку интеграций, что состояние изменилось. */
    onStateChange?: (enabled: boolean, hasError: boolean) => void;
}

/**
 * Настройки связи со службой каталогов.
 *
 ...
 * лишь, задан он или нет, а пустое поле при сохранении оставляет прежний.
 */
export function DirectoryIntegrationForm({onStateChange}: Props) {
    ...
    const apply = useCallback((data: DirectorySettings) => {
        ...
        onStateChange?.(data.enabled, !!data.lastSyncError);
    }, [onStateChange]);
    ...
    const save = () => run(async () => {
        if (!form) return;
        apply(await directorySettingsService.save({...form, serviceAccountPassword: password || null}));
        setPassword("");
    ...
    if (loading) return <div className="text-[13px] text-[#8b97ab]">Загрузка настроек…</div>;
    if (!form || !settings) {
        return <div className="text-[13px] text-[#c0392b]">{error ?? "Настройки недоступны"}</div>;
    }

    return (
        <>
            {error && (
                <div className="mb-4 rounded-[10px] border border-[#f3c9c2] bg-[#fdeeec] px-4 py-2.5 text-[13px] text-[#c0392b]">
                    {error}
                </div>
            )}
            {notice && (
                <div className="mb-4 rounded-[10px] border border-[#cbe6d0] bg-[#f0faf3] px-4 py-2.5 text-[13px] text-[#1f8a4c]">
                    {notice}
                </div>
            )}
            ...
            <div className="rounded-[12px] border border-[#e5e9f0] bg-white p-5">
                <div className="flex items-center justify-between">
                    <div>
                        <h2 className="m-0 text-[15px] font-semibold">Служба каталогов (LDAP)</h2>
                        <div className="mt-0.5 text-[12.5px] text-[#8b97ab]">
                            Пользователи домена подтягиваются в систему по расписанию
                        </div>
                    </div>
